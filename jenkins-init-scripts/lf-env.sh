...
# A library of functions for LF/Jenkins bash scripts. In the general case, in
# general, do NOT set shell variables here. If you want to make a variable
# available to multiple scripts, provide a function/alias that sets the
# variable: 'function lf_set_foo() {foo=asdf;}'. Any scripts that need access to
# the variable can call the 'set' function. This keeps the name-space polution
# to a minimum.
...
################################################################################
# Variables (exceptions to every rule)
################################################################################

# Although this variable is a 'shell' variable, it needs to be upper-case so
# 'shellcheck' does check for 'used-before-set' when they are referenced.

# I am not that this will work in all situations. For now at least, there is
# currently no code that depends on it.

# shellcheck disable=SC2034 # Disable 'unused-varible' check
GLOBAL_JJB_ROOT=$WORKSPACE/global-jjb
