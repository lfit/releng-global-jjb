...
The packer job group contains jobs to validate packer projcet files and build
custom images. The minimal configuration needed to deploy the packer jobs
is as follows which deploys the **{project-name}-packer-jobs** job group as
defined in **lf-ci-jobs.yaml**.
