...
Runs a shell script that creates and uses a Python virtual environment
to build source and (optionally) binary distributions after installing
required packages. Requires a setup.py file at the root of the project
source repository for packaging the component. Writes distribution
files to subdirectory "dist".
...
Runs a shell script that creates and uses a Python virtual environment
to upload distributions to a PyPI index after installing required
packages.  Pushes all distribution files found in subdirectory
"dist". Requires installation of the configuration file ".pypirc".
...
(optionally) a binary distribution. Requires a setup.py file at the
root of the project source repository for packaging the component.
...
PyPI repository. Like the verify job, requires a setup.py file at the
root of the project source repository for packaging the component.
...
Verify job which runs tox, builds source and binary distributions,
verifies that distribution file names contain the release version,
and checks that no tag exists in the code repository for the release
version. See the documentation under "PyPI Merge" about pyenv.

To initiate the release process, create a releases/ or .releases/
directory, add one release yaml file to it, and submit a change set
with that release yaml file to Gerrit. The expected format of the release
yaml file appears in schemas and examples below. The version must be
a valid Semantic Versioning (SemVer) string, matching either the pattern
"v#.#.#" or "#.#.#" where "#" is a one or digits.

This job is similar to the PyPI verify job, but is only triggered by a
release yaml file and performs verification of that file.
...
that distribution file names contain the release version string, checks
that no tag exists in the code repository for the release version, tags
the code repository with the release version, then pushes artifacts to
a named PyPI repository.

This job is similar to the PyPI merge job, but is only triggered by
merge of a release yaml file, and is expected to push to a public
