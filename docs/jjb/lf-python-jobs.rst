...
Verify job which runs tox, then builds source and binary distributions.
...
:Optional Parameters:
    ...
    :parallel: Boolean indicator for tox to run tests in parallel or series.
       (default: false, in series)
    :post_pypi_build_script: Build script to execute after the main builder
        steps. (default: a string with only a comment)
    :pre_pypi_build_script: Build script to execute before the main builder
        steps. For example, move required files to the repo root.
...
:Optional Parameters:
    ...
    :build-timeout: Timeout in minutes before aborting build. (default: 15)
    :git-url: URL clone project from. (default: $GIT_URL/$PROJECT)
    :parallel: Boolean indicator whether to run tests in parallel or series.
       (default: false, in series)
    :post_pypi_build_script: Build script to execute after the main builder
        steps. (default: a string with only a comment)
    :pre_pypi_build_script: Build script to execute before the main builder
