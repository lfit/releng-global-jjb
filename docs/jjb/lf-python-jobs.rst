...
PyPI index. Requires a .pypirc configuration file in the Jenkins builder home directory.
...
PyPI index. Requires a .pypirc configuration file in the Jenkins builder home directory.
Requires a setup.py file at the root of the project source repository.
Very similar to the PyPI Merge job, but uses different triggers and a different default
PyPI index name. Unlike common LF practice of releasing artifacts by moving them from one
place to another, this job re-builds the distributions.
