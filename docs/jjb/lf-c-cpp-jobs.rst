...
Sonar job which runs cmake && make, downloads and installs a Sonar Scanner,
run the scanner, then publishes the result to SonarCloud.

This job purposely runs on the master branch because the basic SonarCloud
configuration does not support multiple branches.
