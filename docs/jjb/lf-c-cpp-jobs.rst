...
:Optional Parameters:
    ...
    :sonar-prescan-script: A shell script to run prior to the Sonar scan.
...
:Required Sonar Properties:
    ...
    - sonar.login: The API token for authentication at SonarCloud.
      Usually defined as key "sonarcloud_api_token" in defaults.yaml.
    - sonar.organization: The umbrella project name; e.g., "opendaylight".
      Usually defined as key "sonarcloud_project_organization" in defaults.yaml.
    ...
    - sonar.projectKey: The globally unique key for identifying the report in
      SonarCloud. Usually the catenation of sonar.organization, an underscore,
