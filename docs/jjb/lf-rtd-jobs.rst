...
well as a token.  The token will be stored in the project Jenkins global-settings
file.  Even though it is a token you will need to put it in a username/password
credentials type to make it available in the global-settings file.  set ```rtd-build-url to the
