...
Calls docker-build.sh script to perform a docker build and
tag construction.
...
:Optional parameters:
    ...
    :docker-tag: Docker tag (or image version) to run the docker build with. (default: latest)
...
Executes a docker build task and pubishes the resulting images.
