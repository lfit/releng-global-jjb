...
for Jenkins Jobs. Global JJB is deployed here as a submodule to allow us easy
management to install, upgrade, and rollback changes via git tags. Global JJB
...
#. Install Global JJB
   ...
   .. note::
      ...
      environmnet it is acceptable to use
...
At this point global-jjb is installed in the ci-management repo and is ready
for use.
...
:templates: A list of templates to build. We recommend only setting one
    template per ``project`` section. That way we can easily control which
    platforms are supported by specific templates.
