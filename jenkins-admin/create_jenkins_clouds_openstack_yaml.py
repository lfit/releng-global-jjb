...
#name: prd- needs to be changable
machinetemplate = """\
          - labels: {{ labels }}
...
for section in r6.sections():
    if section != "cloud":
        ...
        if not "volume_size" in first_dicts:
            first_dicts.update(volume_size="10")
        if not "labels" in first_dicts:
        ...
        #Add snd or prd prefix
        first_dicts.update(name_prefix=name_prefix)
        print(j2_template.render(first_dicts))


#Yeah this needs to go on the end...
#cloud.cfg/CLOUD_ZONE
footertemplate = """\
        zone: {{ cloud_zone}}
"""
