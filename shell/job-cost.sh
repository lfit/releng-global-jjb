...
parse_json_field() {
    ...
    # Use here-string (<<<) instead of echo pipe for robustness with special characters
...
# Flag to track if we have valid instance metadata
metadata_available=true

if ! curl_with_retry "${METADATA_URL}" "instance metadata"; then
    log_error "Unable to retrieve instance type from metadata service"
...
# Skip if we don't have valid instance metadata
if [[ "${metadata_available}" == "true" ]]; then
    ...
    if curl_with_retry "${PRICING_URL}" "Vexxhost pricing API"; then
        ...
        if parse_json_field "${json_block}" ".cost" "cost"; then
            ...
            # Validate that cost is a numeric value before using it
            if [[ "${cost}" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
                log_info "Retrieved cost: ${cost}"
            else
                log_error "Cost value from pricing API is not numeric ('${cost}'), using default: 0"
                cost=0
            fi
        else
    ...
    log_info "Skipping pricing API call - no valid instance metadata"
fi
