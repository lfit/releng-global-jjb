...
packagecloud_verify(){
    ...
    if [[ $1 == $(curl --netrc-file ~/packagecloud_api --silent https://packagecloud.io/api/v1/repos/o-ran-sc/staging/search?q=$1 | yq -r .[].filename) ]]; then
        echo "INFO: $1 exists in staging!"
        echo "INFO: Existing package location: https://packagecloud.io$(curl --netrc-file ~/.netrc --silent https://packagecloud.io/api/v1/repos/o-ran-sc/staging/search?q=$1 | yq -r .[].package_html_url)"
    else
...
packagecloud_promote(){
    ...
    promote_url="https://packagecloud.io$(curl --netrc-file ~/packagecloud_api --silent https://packagecloud.io/api/v1/repos/o-ran-sc/staging/search?q=$1 | yq -r .[].promote_url)"
    echo "INFO: Promoting $1 from staging to release"
    curl --netrc-file ~/packagecloud_api -X POST -F destination=o-ran-sc/release $promote_url | echo "INFO: Promoted package location: https://packagecloud.io$(yq -r .package_html_url)"
}
...
if [[ "$DISTRIBUTION_TYPE" == "maven" ]]; then
    ...
    wget -q https://raw.githubusercontent.com/lfit/releng-global-jjb/master/schema/${RELEASE_SCHEMA}
    verify_schema
    for name in $(cat $release_file | yq -r '.package_name[].name'); do
        package_name=$name
