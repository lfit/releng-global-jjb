...
if (( $(grep -c . <<<"$release_files") > 1 )); then
  ...
  echo "Do not rename or ammend an already merged release file"
  exit 1
else
...
container_release_file(){
  ...
  while read -d $'\n' name version; do
    ...
    if docker pull "$DOCKER_REGISTRY":10001/"$lfn_umbrella"/"$name":"$VERSION"; then
      echo "$VERSION is already released for image $name"
    else
      ...
      echo "Merge will run the following commands:"
      ...
      #Perhaps: docker image inspect $container_image_id:version
