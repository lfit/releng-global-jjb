...
set_variables_common(){
    ...
    printf "\t%-30s %s\n" OVERIDE_SEMVER_REGEX: "$OVERIDE_SEMVER_REGEX"
}
...
verify_version(){
    ...
    OVERIDE_SEMVER_REGEX="${OVERIDE_SEMVER_REGEX:-None}"
    if [[ $OVERIDE_SEMVER_REGEX == "None" ]]; then
        ...
        semver_regex="${OVERIDE_SEMVER_REGEX}"
    fi
