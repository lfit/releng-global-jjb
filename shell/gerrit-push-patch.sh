...
CHANGE_ID=$(ssh -p 29418 "jenkins-$SILO@$GERRIT_HOST" gerrit query \
...
git status
git remote add gerrit "ssh://jenkins-$SILO@$GERRIT_HOST:29418/$PROJECT.git"

# if the reviewers email is empty then use a default
REVIEWERS_EMAIL=${REVIEWERS_EMAIL:-"jenkins-$SILO@$GERRIT_HOST"}
