...
SCRIPT_DIR="$WORKSPACE/archive/groovy-inserts"
SCRIPT_FILE="$SCRIPT_DIR/script.groovy"
...
get_cfg() {
    ...
    cfg=$(grep "${1^^}" "$cfg_file" | tail -1 | awk -F'=' '{print $2}')
    cfg=${cfg:-"$default"}
    echo "$cfg"
}
export get_cfg
...
get_cloud_cfg() {
    ...
    cloud_name=$(get_cfg "$cfg_file" CLOUD_NAME "cattle")
...
get_minion_options() {
    ...
    instance_cap=$(get_cfg "$cfg_file" HARDWARE_ID "20")
...
for cloud in "${clouds[@]}"; do
    ...
    # echo "-----> script.groovy"
    # sed -i "/CLOUD_CONFIG/r $insert_file" "$SCRIPT_FILE"
    # sed -i "0,/CLOUD_CONFIG/{/CLOUD_CONFIG/d}" "$SCRIPT_FILE"

    # TODO: Currently only support a single cloud to be configured so break
    #       after the first cloud is configured.
    break
done

cat "$SCRIPT_DIR/script.groovy"
