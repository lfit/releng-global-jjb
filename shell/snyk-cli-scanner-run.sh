...
if [[ "$JOB_NAME" =~ "docker" ]]; then
    snyk container test "$SNYK_CLI_OPTIONS" "$CONTAINER_PULL_REGISTRY/$DOCKER_NAME:$DOCKER_IMAGE_TAG" --org="$SNYK_ORG"
    snyk container monitor "$SNYK_CLI_OPTIONS" "$CONTAINER_PULL_REGISTRY/$DOCKER_NAME:$DOCKER_IMAGE_TAG" --org="$SNYK_ORG"
else
