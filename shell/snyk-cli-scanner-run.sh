...
if [[ "$JOB_NAME" =~ "docker" ]]; then
    snyk container test opennetworklinux/builder10:1.2 "$SNYK_CLI_OPTIONS" --org="$SNYK_ORG"
    snyk container monitor opennetworklinux/builder10:1.2 "$SNYK_CLI_OPTIONS" --org="$SNYK_ORG"
else
