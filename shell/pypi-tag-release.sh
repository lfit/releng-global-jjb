...
verify_dist(){
    ...
    ls $dir
    file_count=$(find $dir -type f -maxdepth 1 | wc -l)
    vers_count=$(find $dir -type f -maxdepth 1 -name \*$VERSION\* | wc -l)
    if [[ "$file_count" == "$vers_count" ]]; then
        echo "INFO: All file names have expected string ${VERSION}"
    else
        echo "ERROR: found file name(s) without expected string ${VERSION}"
        echo "ERROR: file count total ${file_count}"
        echo "ERROR: file count with version ${vers_count}"
        exit 1
    fi
}
...
tag_gerrit(){
    ...
    if git tag -v "$VERSION"; then
        echo "ERROR: Repo already tagged with $VERSION"
        exit 1
    fi
