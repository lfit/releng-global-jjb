...
verify_dist(){
    ...
    if [[ "$file_count" != "$vers_count" ]]; then
        ...
        echo "ERROR: distribution file total count ${file_count}"
        echo "ERROR: distribution file version count ${vers_count}"
        exit 1
    fi
}
...
if $USE_RELEASE_FILE ; then
    ...
    wget https://raw.githubusercontent.com/lfit/releng-global-jjb/master/schema/${RELEASE_SCHEMA}
    verify_schema
fi
set_variables_pypi
verify_version
