...
template_uuid=$(openstack coe cluster template create "$cluster_template_name" \
  ...
  --flavor "$node_flavor" \
  --docker-volume-size "$volume_size" \
  --network-driver flannel \
  --master-lb-enabled \
  --volume-driver cinder \
  --labels boot_volume_type=ssd,boot_volume_size="${volume_size}",kube_version="${k8s_version}",kube_tag="${k8s_version}" \
  --coe kubernetes \
