...
readarray -t cfg_images <<< "$(grep -r 'ZZCI - ' --include \*.yaml \
    | awk -F": " -e '{print $3}' | sed "s:'::;s:'$::;/^$/d" | sort -u)"
readarray -t yaml_images <<< "$(grep -r IMAGE_NAME --include \*.cfg \
    | awk -F'=' -e '{print $2}' | sort -u)"
IFS=$'\n'
images=($(for R in "${cfg_images[@]}" "${yaml_images[@]}" ; do echo "$R" ; done | sort -u))
unset IFS
