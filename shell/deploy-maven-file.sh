...
# maven2 repository.
#
# $NEXUS_URL_PATH     :  Provided by a job parameter.
#                        The calling job can pass $NEXUS_URL_PATH or local
...
# $UPLOAD_FILES_DIR  :  Provided by a job parameter.
...
do
    lftools deploy maven-file "$NEXUS_URL_PATH" \
                              "$REPO_ID" \
                              "$file" \
                              -b "$MVN" \
                              -g "$GROUP_ID" \
                              ...
                              |& tee "$DEPLOY_LOG"
done < <(find "$UPLOAD_FILES_DIR" -type f -name "*")
