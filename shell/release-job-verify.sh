...
for release_file in $RELEASE_FILES; do
  ...
  if ! [ -z "$MAVEN_CENTRAL_URL" ]; then
    echo "lftools nexus release --server $MAVEN_CENTRAL_URL $STAGING_REPO"
  fi
  ...
  echo "merge would run"
  echo "cd -"
  echo "git checkout "$(echo "awk '{print $NF}' "$PATCH_DIR/taglist.log"")""
  echo "git fetch "$PATCH_DIR/$PROJECT.bundle""
  echo "git merge --ff-only FETCH_HEAD"
  echo "git tag -asm "$PROJECT $VERSION" "v$VERSION""
