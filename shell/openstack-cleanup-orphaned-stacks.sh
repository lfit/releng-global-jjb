...
for cluster_id in "${OS_COE_CLUSTERS_ID[@]}"; do
    # find active stacks associated with the ceo clusters
    stack_id=$(openstack --os-cloud "$os_cloud" coe cluster show "$cluster_id" \
                -f value -c "stack_id" \
                | grep -E '(CREATE_COMPLETE|DELETE_FAILED|UNKNOWN|UNHEALTHY)' \
                | awk '{print $1}')
    OS_COE_STACKS_ID+=("$stack_id")
    echo "clusterid:$cluster_id -> stackid:$stack_id"
done

if [[ ${#OS_COE_STACKS_ID[@]} -gt "0" ]]; then
    echo "${OS_COE_STACKS_ID[@]}"
    echo "-----> Active COE cluster stacks"
    for cstack in "${OS_COE_STACKS_ID[@]}"; do
        echo "$cstack"
    done
fi
...
for STACK_NAME in "${OS_STACKS[@]}"; do
    ...
    if [[ ${#OS_COE_STACKS_ID[@]} -gt "0" ]] && [[ ${OS_COE_STACKS_ID[*]} =~ ${STACK_NAME} ]]; then
