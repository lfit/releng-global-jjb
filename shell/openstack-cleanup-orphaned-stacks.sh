...
stack_in_jenkins() {
    ...
    builds=()
    for jenkins in "${@:2}"; do
        JENKINS_URL="$jenkins/computer/api/json?tree=computer[executors[currentExecutable[url]]"
        JENKINS_URL=$JENKINS_URL",oneOffExecutors[currentExecutable[url]]]&xpath=//url&wrapper=builds"
