...
if [ "${attributes}" == "None" ] && [ "${filters}" == "None" ]; then
    openstack --os-cloud "$os_cloud" "${object}" list -f value -c ID > "$tmpfile"

elif [ "${attributes}" == "None" ]; then
    openstack --os-cloud "$os_cloud" "${object}" list -f value -c ID | "$filters" > "$tmpfile"

elif [ "${filters}" == "None" ]; then
    openstack --os-cloud "$os_cloud" "${object}" list -f value -c ID "$attributes" > "$tmpfile"

else
    openstack --os-cloud "$os_cloud" "${object}" list -f value -c ID "$attributes" | "$filters"  > "$tmpfile"
fi
