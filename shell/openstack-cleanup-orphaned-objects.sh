...
if [[ -n ${filters} ]]; then
    ...
    openstack --os-cloud "$os_cloud" "${object}" list -f value -c ID $attributes | $filters | awk '{print $1}'> "$tmpfile"
else
    # Otherwise don't pipe through an additional command
    openstack --os-cloud "$os_cloud" "${object}" list -f value -c ID $attributes | awk '{print $1}'> "$tmpfile"
fi
