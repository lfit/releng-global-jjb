...
update-python-redhat() {
    sudo /usr/sbin/alternatives  --install /usr/bin/python \
        python /usr/bin/python${SET_PYTHON_VERSION} 310
    sudo /usr/sbin/alternatives --set python3 \
        "/usr/bin/python${SET_PYTHON_VERSION}"
...
update-java-ubuntu() {
    ...
    sudo /usr/bin/update-alternatives --install /usr/bin/python \
        python /usr/bin/python${SET_PYTHON_VERSION} 310
    sudo /usr/bin/update-alternatives --set python "/usr/bin/python3"
    sudo /usr/bin/update-alternatives --set python3 \
        "/usr/bin/python${SET_PYTHON_VERSION}"
...
if  ! command -v "${SET_PYTHON_VERSION}" >/dev/null 2>&1; then
    echo "ERROR: ${SET_PYTHON_VERSION} is not installed."\
        "Upgrade image to newer build"
    exit 1
fi
